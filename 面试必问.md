...
**虚拟机栈(VM Stack)**：与程序计数器一样，虚拟机栈也是线程私有的。线程在执行每一个方法时，都会创建一个栈帧，存放局部变量表、操作数、动态链接、方法出口信息。局部变量表中存储各种基本数据类型以及对象的引用，并且不同的数据变量类型占据不同大小的变量空间(Slot)，一般来说double和long占据2个Slot大小，其他例如int,float,byte等占据一个(当线程请求的栈深度大于允许深度，将抛出StackOverflowError；当虚拟机栈扩展时无法申请到足够的内存，则抛出OutOfMemoryError)；
