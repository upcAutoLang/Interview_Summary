...
1. 服务获取客户端发来的 HTTP 请求，进入 DispatcherServlet#service() 方法，然后进入 **doDispatch()** 方法；
2. 根据 request 信息，获取用于处理请求的**处理器方法 HandlerMethod**；
	- Spring 容器在启动的时候会载入若干 HandlerMapping 实例到 DispatcherServlet 中，调用接口方法 getHandler，该方法会在初始化后的**映射注册器 MappingRegistry** 中寻找匹配的**处理器方法 HandlerMethod**；
3. 根据 HandlerMethod，封装得到统一的接口处理器 HandlerAdapter 中；
...
值得一提的是，执行到最后的 **StandardWrapperValue** 时，会创建一个 FilterChain 用于匹配我们创建的 Fitler 和 Servlet。FilterChain 的作用是通过其 **doFilter** 方法依次调用：
