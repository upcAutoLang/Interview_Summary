...
自研分布式服务治理系统（简称集群管理系统）的实现思想与 Dubbo 基本相同，但我们依然自己选择实现了一套服务治理系统，主要有几个原因：

1. **跨语言调用问题**：Dubbo 只支持 Java 语言；
	- 由于项目遗留问题，有很多 C++ 编写的客户端，而且涉及到各种外单位，协调困难；
2. **时间尴尬**：Dubbo 最早一段时间从 2011 年开始开源，2015 年社区不维护，我们项目从 2016 年开始，所以考虑到如果 Dubbo 有什么问题，可能是隐患，所以没有使用；

系统中的 gRPC 调用流程基本按照 **gRPC 服务端**与 **gRPC 客户端**进行说明，对应了 Dubbo 的服务提供者 (Provider) 与服务消费者 (Consumer)。
...
## 10.2 关于自研分布式服务治理系统的思考

### 10.2.1 注册中心的选择

Zookeeper 保证 CP，而对于服务发现而言，可用性 (Availabe) 比数据一致性 (Consistent) 更重要。目前我们的服务分机房，而且以机房为基本单位，机房内部成一套系统，不存在跨机房的情况，所以没有问题。但如果以后存在多机房服务治理问题时，需要多考虑是否用 Eureka 之类保证 AP 不保证 CP 的注册中心。

### 10.2.2 代理层实现

Dubbo 中的 Proxy 层是使用 JDK 的动态代理实现的，而我们自研的分布式服务治理系统中，使用的方式是**动态生成的静态代理**。将 RPC 实现类输入进 FreeMarker 渲染代码，编译成代理类，加载到类加载器中，完成服务注册、暴露、提供服务等功能。这样实现的原因：

1. 开发时，项目组一开始使用的是 JDK 1.6，当时 JDK 动态代理的性能有瓶颈，所以用 FreeMarker 编译类的这种静态代理方式保证性能。
	- 后来，JDK 版本升级之后，JDK 动态代理性能有显著提升，所以该优点也越来越不明显；
2. 项目组比较熟悉 FreeMarker；

但个人感觉缺点更多：

1. 难以调试；
2. 启动时，编译代码并加载的过程速度较慢；
3. 动态代理性能越来越好，该优点慢慢变少；